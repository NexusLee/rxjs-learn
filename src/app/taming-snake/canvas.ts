...
export function renderGameOver(ctx: CanvasRenderingContext2D){
  ...
  let testX = CANVAS_WIDTH / 2;
  let testY = CANVAS_HEIGHT / 2;

  drawText(ctx, 'GAME OVER!', testX, testY, 'black', 25);
}

function drawText(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, fillStyle: string, fontSize: number, horizontalAlign: string = 'center', verticalAlign: string = 'middle') {
